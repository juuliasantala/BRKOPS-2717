...
def generate_token() -> str:
    ...
    if req.status_code in (200, 201):
        ...
        print(Back.LIGHTRED_EX + ":: ERROR in generate_token() ::")
        print(Back.LIGHTRED_EX + f"The status code is {req.status_code}")
...
def update_interface_status(action_arg, interface_uuid, interface_name, token_arg) -> bool:
    ...
    if req.status_code in (200, 202):
        print(
            Fore.GREEN
            + f"Port {interface_name} with id {interface_uuid} is updated to {new_status}"
        )
        updating_status = True
    elif req.status_code == 500:
        print(
            Fore.RED
            + req.json()["response"]["detail"]
            + f", as the port is already {new_status}"
        )
